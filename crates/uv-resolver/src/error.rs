...
pub struct NoSolutionError {
    ...
    /// filtered by [`EnvVars::UV_INTERNAL__EXCLUDE_NEWER_HINT`] for deterministic test
    /// output). Used by the `exclude-newer` hint to detect versions that were filtered.
...
fn collapse_unavailable_version_chain(
    ...
    let (package, versions, reason, other) = match (&*derived.cause1, &*derived.cause2) {
        (DerivationTree::External(External::Custom(package, versions, reason)), other)
        | (other, DerivationTree::External(External::Custom(package, versions, reason))) => {
            (package, versions, reason, other)
        }
        _ => return None,
    };
