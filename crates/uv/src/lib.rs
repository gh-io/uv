...
/// If `initialize_globals` is `true`, set up process-global state (flags, preview, logging,
/// miette, colors, warnings, ulimit). Set it to `true` on the first call in a process and
/// `false` for any later calls (e.g., from benchmarks that invoke [`run`] in a loop).
#[instrument(skip_all)]
pub async fn run(cli: Cli, initialize_globals: bool) -> Result<ExitStatus> {
    ...
    if initialize_globals && cli.top_level.global_args.quiet == 0 {
        uv_warnings::enable();
    }
    ...
    #[cfg(unix)]
    if initialize_globals && globals.preview.is_enabled(PreviewFeature::AdjustUlimit) {
