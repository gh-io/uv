...
fn initialize_global_state(
    ...
    uv_flags::init(EnvironmentFlags::from(environment))
        .map_err(|()| anyhow::anyhow!("Environment flags are already initialized"))?;
...
/// If `initialize_globals` must be `true` the first time this function is called, and `false` for
/// all subsequent calls.
#[instrument(skip_all)]
pub async fn run(cli: Cli, initialize_globals: bool) -> Result<ExitStatus> {
    ...
    if initialize_globals {
        initialize_global_state(&environment, &globals)?;
    }
    ...
    T: Into<OsString> + Clone,
{
    ...
    let main2 = move || {
        ...
        let result = runtime.block_on(Box::pin(run(cli, true)));
